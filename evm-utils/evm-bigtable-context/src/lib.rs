...
type MemMap<K, V> = Arc<Mutex<BTreeMap<K, V>>>;
...
impl MemMapProvider {
    fn take_map_shared<K: Send + Sync + 'static + Ord, V: Send + Sync + 'static>(
        &mut self,
        column: String,
    ) -> Result<MemMap<K, V>> {
    ...
    fn take_map_unique<K: Ord, V>(_column: String) -> MemMap<K, V> {
